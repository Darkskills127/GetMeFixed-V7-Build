...
gx_camera = {
  ...
  _orbit_yaw = 0,
  _orbit_pitch = 0,
  _max_orbit_pitch = 89.0,
  _min_orbit_pitch = -89.0,
  _keyboard_speed = 0.5,
  _lookat_x = 0,
  _lookat_y = 0,
  _lookat_z = 0,
...
function gx_camera.create_perspective_vk(fov, viewport_x, viewport_y, viewport_width, viewport_height, znear, zfar)
  local aspect = 1.0
  if (viewport_height > 0) then
    aspect = viewport_width / viewport_height
  end
  local camera = gh_camera.create_perspective_vk(fov, aspect, znear, zfar)
  gh_camera.set_viewport(camera, viewport_x, viewport_y, viewport_width, viewport_height)
  gh_camera.set_position(camera, 0, 0, 20)
  gh_camera.set_lookat(camera, 0, 0, 0, 1)
  gh_camera.set_upvec(camera, 0, 1, 0, 0)
...
function gx_camera.update_perspective_vk(camera, fov, viewport_x, viewport_y, viewport_width, viewport_height, znear, zfar)
  local aspect = 1.0
  if (viewport_height > 0) then
    aspect = viewport_width / viewport_height
  end
  gh_camera.update_perspective_vk(camera, fov, aspect, znear, zfar)
  gh_camera.set_viewport(camera, viewport_x, viewport_y, viewport_width, viewport_height)
  gx_camera._fov = fov
  gx_camera._viewport.x = viewport_x
  gx_camera._viewport.y = viewport_y
...
function gx_camera.set_min_max_orbit_pitch_angles(min_pitch, max_pitch)
	gx_camera._min_orbit_pitch = min_pitch
	gx_camera._max_orbit_pitch = max_pitch
end
...
function gx_camera._rotate_camera_orbit(camera, mouse_x, mouse_y, lookat_x, lookat_y, lookat_z)
  ...
  if (gx_camera._orbit_pitch >= gx_camera._max_orbit_pitch) then
		gx_camera._orbit_pitch = gx_camera._max_orbit_pitch
	end
	
	if (gx_camera._orbit_pitch <= gx_camera._min_orbit_pitch) then
		gx_camera._orbit_pitch = gx_camera._min_orbit_pitch
	end
